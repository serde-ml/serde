...
module Ser : Ser.Intf with type output = Tyxml.Xml.elt = Ser.Make (struct
  ...
  let serialize_unit _ser _output () = 
    Ok (Tyxml.Xml.node ~a:[
      Tyxml.Xml.string_attrib "type" "unit";
    ] "prim" [])
  ...
  let serialize_tuple
    ...
    Ok (Tyxml.Xml.node ~a:[Tyxml.Xml.string_attrib "type" "tuple"] "prim" parts)
  ...
  let serialize_record_variant
    ...
    let fields = List.map (fun (name, xml) ->
      Tyxml.Xml.node name [xml]
    ) fields in
